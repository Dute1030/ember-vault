...
test('AOE magic affects every living enemy and does not mutate game state',()=>{const g=combat();g.enemies.push(structuredClone(g.enemies[0]));g.hand=['nova'];g.focus=3;const a={type:'play',index:0} as const,n=act(g,a),snapshot=structuredClone(n);const fx=combatFeedback(g,n,a);assert.deepEqual(fx.impacts.filter(i=>i.kind==='magic').map(i=>[i.target,i.amount]),[[0,25],[1,25]]);assert.deepEqual(n,snapshot)});
