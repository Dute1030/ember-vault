...
- **每职业两条构筑路线**：蓄甲／血铸、叠毒／连击、蓄能／寒霜；组合奖励要求先建立护甲、支付生命或施加状态。选牌界面展示已有配合，详见 [流派指南](docs/archetypes.md)。
- **白／紫／金三档稀有度**：30／18／12 张，不同战斗和商店有不同获取概率，售价分别为 40／65／100 金币；详见 [稀有度设计](docs/card-rarity.md)。
- **60 张卡牌，每张两条升级路线**：强化数值，或选择减费、穿甲、群体效果、保留护甲等专精机制；升级前后对比，确认后生效。
- **15 层连线路线图与 3 位 Boss**：破盾打断、祷告召唤、半血后限制连出四张牌。
...
- **场景 BGM 与独立音效开关**：探索、战斗、Boss、商店、营地使用不同原创循环曲，切换时渐变过渡，无外部音频请求。
...
当前包含 96 项测试。默认模拟每职业每难度 100 局，可调整：
...
最新流派版本使用三种抓牌策略与可选两步出牌预判验证，详见 [流派设计与模拟说明](docs/archetypes.md)。此前稀有度版本的 22.3% 为旧版本、旧策略数据，不能代表当前胜率。模拟不保证真人通关率。
...
React 19、TypeScript、Vite/Vinext、Tailwind CSS、Base UI 与 Lucide 图标。背景及像素角色图集为本项目生成的素材；BGM 由 `scripts/generate_bgm.py` 与 `scripts/generate_scene_bgm.py` 合成，包含探索及四首场景循环曲，无外部采样。战斗音效使用 Web Audio 即时合成。
