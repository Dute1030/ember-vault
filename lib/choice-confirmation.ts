...
import {cardPrice,RARITIES,cardRemovalCost,shopRefreshCost,getCard,description,NODE_NAMES,type Game,type Action} from './game.ts';
...
case 'buy':title=action.index===3?'购买随机遗物':'购买 '+getCard(state.shop[action.index]).name+' · '+RARITIES[getCard(state.shop[action.index]).rarity].name;detail=action.index===3?'支付 90 金币，获得一件随机遗物。':`${getCard(state.shop[action.index]).cost} 能量。`+description(getCard(state.shop[action.index]))+` 支付 ${cardPrice(state.shop[action.index])} 金币并加入牌组。`;break;
